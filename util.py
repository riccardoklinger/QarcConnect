    ...
    try:
        ...
        if doc.tag != "qgsCSWConnections":
            ...
            msg = QCoreApplication.translate(
                "QarcConnectDialog", "Invalid Catalog connections XML."
            )
    except etree.ParseError as err:
        error = 1
        msg = QCoreApplication.translate(
            "QarcConnectDialog", "Cannot parse XML file: {0}"
        ).format(err)
    except OSError as err:
        error = 1
        msg = QCoreApplication.translate(
            "QarcConnectDialog", "Cannot open file: {0}"
        ).format(err)

    if error == 1:
        QMessageBox.information(
            parent,
            QCoreApplication.translate("QarcConnectDialog", "Loading Connections"),
            msg,
        )
        return
    return doc
...
def get_help_url():
    """return QGIS QarcConnect help documentation link"""
    ...
    path = f"{version}/{locale_name}/docs/user_manual/plugins/core_plugins/plugins_QarcConnect.html"  # noqa
...
def log_message(message, level=Qgis.MessageLevel.Info):
    ...
    logging = QgsSettings().value("/QarcConnect/logDebugging", False, bool)
    if logging:
        LOGGER.logMessage(message, "QarcConnect", level)
