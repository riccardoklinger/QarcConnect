...
from QarcConnect.util import get_ui_class
from QarcConnect.search_backend import CATALOG_TYPES
...
class NewConnectionDialog(QDialog, BASE_CLASS):
    ...
    def accept(self):
        ...
        if conn_name is not None:
            key = "/QarcConnect/%s" % conn_name
            keyurl = "%s/url" % key
            key_orig = "/QarcConnect/%s" % self.conn_name_orig
            ...
            self.settings.setValue("/QarcConnect/selected", conn_name)
