...
from QarcConnect.util import get_connections_from_file, get_ui_class, prettify_xml
...
class ManageConnectionsDialog(QDialog, BASE_CLASS):
    ...
    def save(self, connections):
        ...
        for conn in connections:
            url = self.settings.value("/QarcConnect/%s/url" % conn)
            type_ = self.settings.value("/QarcConnect/%s/catalog-type" % conn)
            if url is not None:
    ...
    def load(self, items):
        ...
        for catalog in exml.findall("csw"):
            ...
            url = "/QarcConnect/%s/url" % conn_name
            type_ = "/QarcConnect/%s/catalog-type" % conn_name
